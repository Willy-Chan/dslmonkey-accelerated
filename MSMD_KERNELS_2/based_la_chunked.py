...
### TODO: THIS IS THE UNFUSED VARIANT
# applied unfused kernel writer to 5_4_18, 5_5_17, 5_6_4, 5_7_2, 5_8_3, etc.s
...
def _build_linear_state_kernel(batch, heads, seq_len, feat_dim, head_dim, chunk_size, dtype="float16"):
    ...
    def kernel_func(
        ...
        with T.Kernel(total_seqs, threads=128) as bx:
            ...
            # TODO: "Manual" intervention: I have to put this if-statement because the compiler won't let me swizzle small tensors needed for the benchmark
...
def _build_linear_proj_kernel(batch, heads, seq_len, feat_dim, head_dim, chunk_size, dtype="float16"):
    ...
    def kernel_func(
        ...
        with T.Kernel(grid_x, total_chunks, threads=128) as (bx, by):
            ...
            # TODO: "Manual" intervention: I have to put this if-statement because the compiler won't let me swizzle small tensors needed for the benchmark
            # NOTE: For small `feat_dim` (e.g. 16), forcing a swizzled layout can conflict with
            # TileLang's GEMM layout inference (observed as "Get different layout for Q_shared").
            # Only apply swizzled layouts when both operands are large enough.
            if feat_dim >= 64 and head_dim >= 64:
                T.annotate_layout({
